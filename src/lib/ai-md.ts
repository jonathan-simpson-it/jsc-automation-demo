...
export function buildHomeMd() {
  ...
  sections.push('## Engagement model')
  sections.push('')
  sections.push('Bespoke per build: setup and integration scoped after a compliance diagnostic, with recurring backend and server hosting included. How we work:')
  sections.push('')
  for (const step of c.pricing.processSteps) {
    sections.push(`- **${step.title}**: ${step.body}`)
  }
  sections.push('')
  ...
  for (const item of c.roadmap.items) {
    sections.push(`- **${item.label}** (${item.pct}% of grant): ${item.description}`)
  }
  sections.push('')
